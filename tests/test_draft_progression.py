...
scenarios = [
    ...
    },
    {
        ...
        "roster": {"QB": [], "RB": ["rb1"], "WR": ["wr1"], "TE": [], "FLEX": [], "K": [], "DST": [], "BENCH": []},
        "round": 3,
    },
    {
...
for scenario in scenarios:
    ...
    for i, player in enumerate(sorted_players[:5], 1):
        ...
        if player_data is None:
            continue
        print(f"{i:<6} {player_data['name']:<25} {player.position:<5} "
...
for drafted in drafted_rbs:
    ...
    if rbs:
        ...
        effect = f"{'+' if drafted > 0 else ''}{((top_rb.dynamic_vor - 754.64) / 754.64 * 100):.1f}%"
        print(f"{drafted:<15} {top_rb.base_vor:<15.2f} {top_rb.scarcity_multiplier:<15.2f} "
